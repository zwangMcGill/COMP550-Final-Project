...
def get_nlp_features_fixed_length(layer, seq_len, feat_type, feat_dir, train_indicator, SKIP_WORDS=20, END_WORDS=985):
    # 得到的bert layer representation
    # train/test 都是从 layer representation 得出
    # 可以被重写
    loaded = np.load(feat_dir + feat_type + '_length_' + str(seq_len) + '_layer_' + str(layer) + '.npy') # 5176
    if feat_type == 'elmo':
...
def TR_to_word_CV_ind(TR_train_indicator, SKIP_WORDS=20, END_WORDS=985):
    time = np.load('./data/fMRI/new_time_fmri.npy')
    runs = np.load('./data/fMRI/new_runs_fmri.npy')
    time_words = np.load('./data/fMRI/new_time_words_fmri.npy')
    ...
    # print("word_train_indicator", len(word_train_indicator))
    # print(word_train_indicator)
    return word_train_indicator


def prepare_fmri_features(train_features, test_features, word_train_indicator, TR_train_indicator, SKIP_WORDS=20,
                          END_WORDS=985):
    time = np.load('./data/fMRI/new_time_fmri.npy')  # fMRI data was acquired at a rate of 2s per image  1350
    runs = np.load('./data/fMRI/new_runs_fmri.npy')  # 1-4 1350
    time_words = np.load('./data/fMRI/new_time_words_fmri.npy')  # word present one at a time at a rate of 0.5s 5176
    time_words = time_words[SKIP_WORDS:END_WORDS]

    words_id = np.zeros([len(time_words)]) #5156
    # w=find what TR each word belongs to
    # every four words have the same id
    for i in range(len(time_words)):
        words_id[i] = np.where(time_words[i] > time)[0][-1] ## 最后一个id：1346
        # print(np.where(time_words[i] > time))

    np.set_printoptions(threshold=np.inf)
    all_features = np.zeros([time_words.shape[0], train_features.shape[1]])  # 5156 10
    # word_train_indicator: True/False

    # test and train features are good
    # word_train_indicator might be wrong
    all_features[word_train_indicator] = train_features
    all_features[~word_train_indicator] = test_features
    print("all_feature",len(all_features))
    p = all_features.shape[1]  # 10
    tmp = np.zeros([time.shape[0], p])  # 1351 10

    # for i in range(700,925):
    #     print(i)
    #     print(all_features[(words_id <= i) * (words_id > i - 1)])
    ...
    #print("this is in prepare_fmri_features")
    #885 2658
    print(len(tmp[TR_train_indicator]))
...
def run_class_time_CV_fmri_crossval_ridge(data, predict_feat_dict,
    ...
    ind = CV_ind(n_words, n_folds=n_folds)  # 1211 0:3
    # print("ind", ind)
    # correct value derived from fMRI images
    ...
    for ind_num in range(1):
        train_ind = ind == ind_num
        test_ind = ind != ind_num
        # print("train_ind",len(train_ind))
        word_CV_ind = TR_to_word_CV_ind(train_ind)  # 5156 T/F # 转为 965个 T/F
        #print(len(word_CV_ind))
        _, _, tmp_train_features, tmp_test_features = get_nlp_features_fixed_length(layer, seq_len, nlp_feat_type,
                                                                                    feat_dir, word_CV_ind)
        # tmp_feature 是正确的
        train_features, test_features = prepare_fmri_features(tmp_train_features, tmp_test_features, word_CV_ind,
                                                              train_ind)
        # split data
        ...
        if ind_num == 0:  # just remove from front end
            ...
            test_data = train_data[skip:-skip, :]
            test_features = train_features[skip:-skip, :]
        ...
        start_time = tm.time()
        weights, chosen_lambdas = cross_val_ridge(train_features, train_data, n_splits=10,
    ...
    # print(np.vstack(all_test_data))
    print(np.vstack(all_preds))
...
def binary_classify_neighborhoods(Ypred, Y, n_class=20, nSample=1000, pair_samples=[], neighborhoods=[]):
    ...
    # print("neightbourhoods",neighborhoods[0])
    import time as tm
    ...
    for idx in range(nSample):
        ...
        # print("dist_correct", len(dist_correct))
        # print("voxels", voxels)

        # # @test
        # for v in range(voxels):
        #     print(v)
        #     print(neighborhoods[v, neighborhoods[v, :] > -1])
        #     np.sum(dist_correct[neighborhoods[v, neighborhoods[v, :] > -1]])

        neighborhood_dist_correct = np.array(
            [np.sum(dist_correct[neighborhoods[v, neighborhoods[v, :] > -1]]) for v in range(voxels)])
        neighborhood_dist_incorrect = np.array(
            [np.sum(dist_incorrect[neighborhoods[v, neighborhoods[v, :] > -1]]) for v in range(voxels)])

        acc[idx, :] = (neighborhood_dist_correct < neighborhood_dist_incorrect) * 0.5 + (
                neighborhood_dist_correct == neighborhood_dist_incorrect) * 1.0
