...
from utils.gpt2_utils import get_gpt2_layer_representations
...
def save_layer_representations(model_layer_dict, model_name, seq_len, save_dir):
    for layer in model_layer_dict.keys():
        np.save('{}/{}_length_{}_layer_{}.npy'.format(save_dir, model_name, seq_len, layer + 1),
                np.vstack(model_layer_dict[layer]))
    print('Saved extracted features to {}'.format(save_dir))
    return 1


model_options = ['bert', 'transformer_xl', 'elmo', 'use', 'gpt2']
...
if __name__ == '__main__':
    ...
    parser.add_argument("--sequence_length", type=int, default=1,
                        help='length of context to provide to NLP model (default: 1)')
    ...
    if args.nlp_model == 'bert':
        ...
        nlp_features = get_bert_layer_representations(args.sequence_length, text_array, remove_chars,
                                                      word_ind_to_extract)
        ...
        word_ind_to_extract = -1
        nlp_features = get_elmo_layer_representations(args.sequence_length, text_array, remove_chars,
                                                      word_ind_to_extract)
    elif args.nlp_model == 'use':
        nlp_features = get_use_layer_representations(args.sequence_length, text_array, remove_chars)

    elif args.nlp_model == 'gpt2':
        word_ind_to_extract = -2
        nlp_features = get_gpt2_layer_representations(args.sequence_length, text_array, remove_chars,
                                                      word_ind_to_extract)
    elif args.nlp_model == 'xl_net':
        word_ind_to_extract = -1
        nlp_features = get_xl_net_layer_representations(args.sequence_length, text_array, remove_chars,
                                                        word_ind_to_extract)
    else:
